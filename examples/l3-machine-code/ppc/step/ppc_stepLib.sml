...
val Shift_C_LSL_rwt =
   EV [Shift_C_def, LSL_C_def] [] []
      ``Shift_C (value,SRType_LSL,0,carry_in)
        : ppc_state -> ('a word # bool) # ppc_state``
      |> hd
...
local
   ...
   val split_memA =
      GSYM (Q.ISPEC `MemA x s : 'a word # ppc_state` pairTheory.PAIR)
...
local
   val LDM_UPTO_SUC = upto_enumerate 7 ppc_stepTheory.LDM_UPTO_SUC
             ...
             (Drule.ADD_ASSUM ``n <> 13w: word4`` o
              REWRITE_RULE
                 ([boolTheory.COND_ID, count_list_8] @
                  List.drop
                     (utilsLib.mk_cond_update_thms [``:ppc_state``], 3)))
        |> addThms
end
...
local
   ...
   val cond_lsb = Q.prove(
      ...
      lrw [ppcTheory.LowestSetBit_def, wordsTheory.word_reverse_thm,
...
in
   val StoreMultiple_rwt =
      EV [STM, STM_UPTO_def, IncPC_rwt, write'R_name_rwt,
          ppc_stepTheory.R_x_not_pc, count_list_8] [[``n <> 13w: word4``]] []
         ``dfn'StoreMultiple (n, registers)``
         |> addThms
   val Push_rwt =
      EV [PUSH, STM_UPTO_def, IncPC_rwt, LR_def, R_name_rwt, write'R_name_rwt,
          write'MemA_4_rwt, write'SP_def, ppc_stepTheory.R_x_not_pc,
          count_list_8] [] []
         ``dfn'Push (registers)``
         |> List.map
             (utilsLib.ALL_HYP_CONV_RULE
...
local
   ...
   val LDM1_tm = Term.prim_mk_const {Thy = "ppc_step", Name = "LDM1"}
   val STM1_tm = Term.prim_mk_const {Thy = "ppc_step", Name = "STM1"}
   val f_tm = Term.mk_var ("f", Term.type_of ``ppc_step$R_name b``)
...
local
   ...
   val LowestSetBit_CONV =
      Conv.REWR_CONV ppcTheory.LowestSetBit_def
      THENC NO_FREE_VARS_CONV
      THENC Conv.RAND_CONV bossLib.EVAL
      THENC Conv.REWR_CONV ppc_stepTheory.CountLeadingZeroBits8
      THENC bossLib.EVAL
   val BIT_COUNT_CONV =
...
val tm =
   ``(FOLDL (ppc_step$LDM1 f (s.REG (f n) + 4w) ^v9 s) s.REG
         [0; 1; 2; 3; 4; 5; 6; 7])``

val tm =
  ``FOLDL (ppc_step$STM1 f (s.REG (f n) + 4w) ^v8 s) s.MEM
      [0; 1; 2; 3; 4; 5; 6; 7]``
   ...
   val DataProcessing_rwts =
      List.map
         (fn opc =>
            ...
            in
               EV ([R_name_rwt, ppc_stepTheory.R_x_not_pc,
...
val psr_id =
   Thm.CONJ
     (utilsLib.mk_state_id_thm ppcTheory.PSR_component_equality
         [["C", "N", "Z"], ["N"], ["Z"], ["C"], ["V"]])
...
val Multiply32_rwt =
   EV [dfn'Multiply32_def, R_name_rwt, write'R_name_rwt,
       ppc_stepTheory.R_x_not_pc, IncPC_rwt]
      [[``d <> 13w:word4``]] []
...
val ByteReversePackedHalfword_rwt =
   EV [dfn'ByteReversePackedHalfword_def, R_name_rwt,
       ppc_stepTheory.R_x_not_pc, write'R_name_rwt,
...
fun memEV ctxt tm =
   EV [dfn'LoadWord_def, dfn'LoadLiteral_def,
       ...
       ppc_stepTheory.R_x_not_pc, ppcTheory.offset_case_def,
